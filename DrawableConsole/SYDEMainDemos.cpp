...
void SYDEMainDemos::GenerateTriangle()
{
...
void SYDEMainDemos::TicTacToe(bool isAIp1, bool isAIp2, int offset, string OBrain, string XBrain)
{
...
void SYDEMainDemos::PixelArt(ColourClass skinOuter, ColourClass skinMiddle, ColourClass skinInner, ColourClass Meat, ColourClass Seed, ColourClass BG, string a_text)
{
...
void SYDEMainDemos::Options(vector<string> options)
{
	//window.ClearWindow(false);
	CONSOLE_CURSOR_INFO cInfo;
	GetConsoleCursorInfo(hOut, &cInfo);
...
void SYDEMainDemos::generateart()
{
	int asciiArt = rand() % 3;
	window.ClearWindow(true);
	switch (asciiArt)
...
void SYDEMainDemos::drawBrain(string AIBrain)
{
	for (int l = 0; l < windowWidth; l++)
	{
		for (int m = 0; m < windowHeight; m++)
...
void SYDEMainDemos::playTicTacToe()
{
	int gameNO = 0;
	int gamesMax = 5;
...
ColourClass SYDEMainDemos::determineColour(ColourClass main, ColourClass bg)
{
...
void SYDEMainDemos::PlayLevel(string level, bool jumpAllowed, bool dropAllowed)
{
	//window.ClearWindow(true);
	//for (int l = 0; l < windowWidth; l++)
	//{
...
void SYDEMainDemos::bmp_test2(CustomAsset asset_to_draw)
{
...
void SYDEMainDemos::animation_test()
{
...
void SYDEMainDemos::animation_test_turtle()
{
	LPCWSTR title = L"Animation Test"; //GOOD TITLE, NO CHANGE NEEDED BOSS
	SetConsoleTitleW(title);
	window.ClearWindow(true);
...
void SYDEMainDemos::Sprite_Sheet_Test()
{
...
void SYDEMainDemos::animation_test_player()
{
	string lastPress = "Null";
	LPCWSTR title = L"Animation Test"; //GOOD TITLE, NO CHANGE NEEDED BOSS
	SetConsoleTitleW(title);
...
void SYDEMainDemos::set_up_custom_assets()
{
...
void SYDEMainDemos::set_up_window_properties()
{
...
void SYDEMainDemos::set_up_shape_test()
{
...
vector<string> SYDEMainDemos::Split(string a_String, char splitter)
{
	int arraySize = 1;
...
bool SYDEMainDemos::charHitBoxCheck(Vector2 point, Vector2 obj, int startX, int endX, int startY, int endY)
{
...
void SYDEMainDemos::introductionScript()
{
...
string SYDEMainDemos::returnRandomNonsense()
{
	string pText = "";
...
void SYDEMainDemos::drawTitle(int baseY, int baseX)
{
	//LINE 1
...
void SYDEMainDemos::drawBee(int baseY, int baseX, bool setUp)
{
	if (setUp)
	{
		window.ClearWindow(true);
...
void SYDEMainDemos::poweredBySYDEEngine(int baseY, int baseX)
{
	CONSOLE_CURSOR_INFO cInfo;
	GetConsoleCursorInfo(hOut, &cInfo);
	cInfo.bVisible = false;
...
void SYDEMainDemos::introCreditsScript()
{
	window.ClearWindow(true);
	cout << "SYDE" << endl;
	Sleep(25);
...
wstring SYDEMainDemos::s2ws(const string& s)
{
	wstring r;
