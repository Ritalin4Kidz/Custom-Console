...
{
	if (!halfTimeHasPassed && m_MinutesPassed >= m_TimePerHalf)
	{
		return to_string(m_TimePerHalf) + ":00";
	}
	if (!fullTimeHasPassed && m_MinutesPassed >= (m_TimePerHalf *2))
	{
		return to_string(m_TimePerHalf*2) + ":00";
	}
...
void SRLGameManager::addMinute()
{
	...
	if (!halfTimeHasPassed && m_MinutesPassed >= m_TimePerHalf)
	{
		m_MinutesPassed = m_TimePerHalf;
		timeToAddPlayers -= m_SecondsPassed;
		m_SecondsPassed = 0;
	}
	if (!fullTimeHasPassed && m_MinutesPassed >= (m_TimePerHalf*2))
	{
		m_MinutesPassed = (m_TimePerHalf*2);
		timeToAddPlayers -= m_SecondsPassed;
		m_SecondsPassed = 0;
	}
...
bool SRLGameManager::addTime(int seconds, bool allowBreak)
{
	...
	if (m_SecondsPassed >= 60)
	{
		...
		{
			if (m_MinutesPassed >= m_TimePerHalf)
			{
				m_MinutesPassed = m_TimePerHalf;
				m_SecondsPassed = 0;
				seconds = ((m_TimePerHalf * 60) - currentTime);
				if (allowBreak)
				{
					halfTimeHasPassed = true;
					m_PlayByPlay.push_back(to_string(m_TimePerHalf) +":00 - HALF TIME - " + m_HomeTeam.getName() + ": " + to_string(homeTeamScore) + " v " + m_AwayTeam.getName() + ": " + to_string(awayTeamScore));
					m_BallPosition = 0;
		...
		{
			if (m_MinutesPassed >= (m_TimePerHalf*2))
			{
				m_MinutesPassed = (m_TimePerHalf*2);
				m_SecondsPassed = 0;
				seconds = ((m_TimePerHalf*2*60) - currentTime);
				if (allowBreak)
				{
					if (m_ExtraTime && homeTeamScore == awayTeamScore)
					{
						m_PlayByPlay.push_back(to_string(m_TimePerHalf*2) +":00 - EXTRA TIME - " + m_HomeTeam.getName() + ": " + to_string(homeTeamScore) + " v " + m_AwayTeam.getName() + ": " + to_string(awayTeamScore));
					}
...
void SRLGameManager::play()
{
	...
	else if (doTwoPointFieldGoal(defender, attacker))
	{
		return;
	}
	else
...
{
	if (m_MinutesPassed > ((m_TimePerHalf * 2) - (((m_TimePerHalf*2)/10) * 2)))
	{
	...
	else if (m_MinutesPassed >= (m_TimePerHalf-2) && m_MinutesPassed < m_TimePerHalf || (m_MinutesPassed == m_TimePerHalf && m_SecondsPassed == 0))
	{
		if ((m_Tackle == 5 && ((homeTeamScore - awayTeamScore >= 0) || homeTeamScore - awayTeamScore == -1)) || homeTeamScore == awayTeamScore)
		{
			if (m_HomeTeamHasBall)
			{
				return doFieldGoalAttempt(attacker, m_HomeTeam, m_AwayTeam);
		...
		else if ((m_Tackle == 5 && ((awayTeamScore - homeTeamScore >= 0) || awayTeamScore - homeTeamScore == -1)) || homeTeamScore == awayTeamScore)
		{
			if (!m_HomeTeamHasBall)
			{
				return doFieldGoalAttempt(attacker, m_AwayTeam, m_HomeTeam);
...
bool SRLGameManager::doTwoPointFieldGoal(SRLPlayer defender, SRLPlayer attacker)
{
	if (m_MinutesPassed >= ((m_TimePerHalf*2) - 5) && m_MinutesPassed <= (m_TimePerHalf*2))
	{
		if (homeTeamScore - awayTeamScore == -2)
		{
			if (m_HomeTeamHasBall)
			{
				return do2PointFieldGoalAttempt(attacker, m_HomeTeam, m_AwayTeam);
			}
		}
		else if (awayTeamScore - homeTeamScore == -2)
		{
			if (!m_HomeTeamHasBall)
			{
				return do2PointFieldGoalAttempt(attacker, m_AwayTeam, m_HomeTeam);
			}
		}
	}
	return false;
...
bool SRLGameManager::do2PointFieldGoalAttempt(SRLPlayer attacker, SRLTeam& attackingTeam, SRLTeam& defendingTeam)
{
	if (getPositionUniversal(m_BallPosition) < 50 && getPositionUniversal(m_BallPosition) > 39)
	{
		...
		{
			addScoreUniversal(2);
			addPlay("2 POINT FIELD GOAL - " + m_HomeTeam.getName() + ": " + to_string(homeTeamScore) + " v " + m_AwayTeam.getName() + ": " + to_string(awayTeamScore), attacker);
			addSummary("2 POINT FIELD GOAL#" + to_string(homeTeamScore) + " v " + to_string(awayTeamScore), attacker);
			setPositionUniversal(100);
			attackingTeam.addPlayerFieldGoal(attacker.getName());
		}
		else
		{
			addPlay("2 POINT FIELD GOAL MISSED", attacker);
			addPlay("20m Restart - " + defendingTeam.getName());
			addSummary("2 POINT FIELD GOAL MISSED#" + to_string(homeTeamScore) + " v " + to_string(awayTeamScore), attacker);
			setPositionUniversal(20);
			changeOver(true);
		}
		m_Tackle = 0;
